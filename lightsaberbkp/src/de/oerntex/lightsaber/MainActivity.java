...
public void buttonPlus(View v)
{
	...
	//fragment manager: neue seite
	//FragmentManager fragmentManager = getFragmentManager();
	//Fragment newFragment = new oneline();fragmentManager.beginTransaction().add(R.id.linecontainer, newFragment, tag).commit();
	//fragmentManager.executePendingTransactions();
...
public boolean sendDataToArduino()
{
	...
	for(int i=1;i<=seitenAnzahl;i++)
	{
		...
		for(int l=1;l<=lineAnzahl;l++)	{
			...
			height=ash.getInt("line"+l+"|heightfield", 1);
			...
			bigLedArray[0]=Color.red(colorToWork);
			bigLedArray[1]=Color.green(colorToWork);
			bigLedArray[2]=Color.blue(colorToWork);
			//line
			bigLedArray[3]=l;
			//height
		...
		//try {Thread.sleep(seitenLaenge*1000);} catch (InterruptedException e) {e.printStackTrace();}
	}
	return true;
}
